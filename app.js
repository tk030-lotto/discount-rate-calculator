/**
 * 蜑ｲ蠑慕紫繧峨￥繧峨￥險育ｮ・- Web Application Logic
...
(() => {
  ...
  const calculate = () => {
    ...
    cleanDiscounts.forEach((rate, idx) => {
      ...
      steps.push({
        step: idx + 1,
        title: idx === 0 ? `${rate}% OFF` : `縺輔ｉ縺ｫ ${rate}% OFF`,
        rate: rate,
  ...
  const render = () => {
    ...
    if (calcResult.cleanDiscounts.length >= 2 && calcResult.cleanDiscounts.some(d => d > 0)) {
      ...
      const firstStepText = `${calcResult.cleanDiscounts[0]}%蠑輔″`;
      const nextStepsText = calcResult.cleanDiscounts.slice(1).map(d => `${d}%蠑輔″`).join('縲√＆繧峨↓');
      
      elMisconceptionText.innerHTML = `
        蜑ｲ蠑募ｾ後・萓｡譬ｼ縺ｫ蟇ｾ縺励※谺｡縺ｮ蜑ｲ蠑輔′鬆・分縺ｫ驕ｩ逕ｨ縺輔ｌ繧九◆繧・ｼ・{firstStepText} 竊・${nextStepsText}・峨・br>
        螳溯ｳｪ逧・↑蜑ｲ蠑慕紫縺ｯ <strong>${formatPercent(calcResult.effectiveRate)}% OFF</strong>・按･${formatCurrency(calcResult.savedAmount)} 蠑輔″・峨→縺ｪ繧翫∪縺吶・      `;
    } else {
  ...
  const renderTimeline = (calcResult) => {
    ...
    calcResult.steps.forEach((step, idx) => {
      ...
      if (!isInitial) {
        ...
        mult.textContent = `ﾃ・${step.multiplier.toFixed(2)}`;
        ...
        diff.textContent = `-ﾂ･${formatCurrency(roundedDiff)}`;
  ...
  const renderDiscountInputs = () => {
    ...
    state.discounts.forEach((discount, idx) => {
      ...
      label.innerHTML = `<span class="step-badge">${idx + 1}</span> 蜑ｲ蠑・${idx + 1}`;
      ...
      input.setAttribute('aria-label', `蜑ｲ蠑・${idx + 1} 縺ｮ繝代・繧ｻ繝ｳ繝・・繧ｸ`);
  ...
  const copyResultToClipboard = async () => {
    ...
    const discountsText = calc.cleanDiscounts.map((d, i) => `${i + 1}谿ｵ髫守岼: ${d}% OFF`).join('\n');
    
    const text = [
      '縲仙牡蠑慕紫繧峨￥繧峨￥險育ｮ・邨先棡縲・,
      `蜈・ｾ｡譬ｼ: ﾂ･${formatCurrency(calc.validPrice)}`,
      '驕ｩ逕ｨ蜑ｲ蠑・',
      discountsText,
      '------------------------',
      `譛邨よ髪謇暮｡・ ﾂ･${formatCurrency(calc.finalPrice)}`,
      `螳溯ｳｪ蜑ｲ蠑慕紫: ${formatPercent(calc.effectiveRate)}% OFF`,
      `縺雁ｾ鈴｡・ ﾂ･${formatCurrency(calc.savedAmount)} 蠑輔″`,
      '------------------------',
      '#蜑ｲ蠑慕紫繧峨￥繧峨￥險育ｮ・
    ].join('\n');
  ...
  const shareConditions = async () => {
    ...
    try {
      ...
      showToast('迫 譚｡莉ｶ莉倥″URL繧偵さ繝斐・縺励∪縺励◆・・);
    } catch (err) {
      showToast('URL縺ｮ繧ｳ繝斐・縺ｫ螟ｱ謨励＠縺ｾ縺励◆縲・);
    }
  };
  ...
  const initEvents = () => {
    ...
    elBtnReset.addEventListener('click', () => {
      ...
      renderDiscountInputs();
      render();
      showToast('売 蜈･蜉帙ｒ蛻晄悄迥ｶ諷九↓繝ｪ繧ｻ繝・ヨ縺励∪縺励◆');
    });
